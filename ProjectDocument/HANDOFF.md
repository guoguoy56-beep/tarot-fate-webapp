...
- `/api/reading` 已完成卡牌身份、牌位、方向、三牌唯一性和服务端可信牌义边界，但仍缺少问题/请求体规模限制、频率限制与成本保护；API-003～005 必须继续完成公网加固。
...
1. `API-003`：限制问题和请求规模，明确字符、空白与换行规则，在旧前端提供必要反馈，并由服务端执行不可绕过的硬限制。

API-002 已完成可信边界收敛：浏览器不再发送牌义，服务端按 `cardId` 查询本地数据并重建内部请求；直接篡改或附加客户端牌名、关键词和含义不能改变 DeepSeek 使用的真实牌义。

`BASE-001`～`BASE-005`、`KIT-001`、`API-001` 和 `API-002` 均已完成。除上述唯一当前任务 `API-003` 外，不并行启动后续 API 任务或新前端设计实现。API-003 只处理长度与请求规模，不提前实现 API-004 限流或 API-005 成本熔断。前端整体重做继续冻结，只有在工程、API、数据、测试与 AI 可用性门禁全部满足后，才进入新前端专项讨论与建设。
