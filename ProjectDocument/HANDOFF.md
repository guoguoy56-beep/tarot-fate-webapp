...
- `/api/reading` 已完成卡牌身份、牌位、方向、三牌唯一性、服务端可信牌义、问题长度和请求体规模限制，但仍缺少部署有效的频率/并发限制与成本保护；API-004～005 必须继续完成公网加固。
...
1. `API-004`：先确定目标部署环境和客户端标识信任边界，再实现匿名频率、时间窗口、并发约束与稳定 429。

API-003 已完成字符和字节双边界：500 字问题与 4096 字节请求体采用浏览器提示、服务端独立校验和流式实际字节计数，无法仅通过绕过前端或省略 `Content-Length` 逃避限制。

`BASE-001`～`BASE-005`、`KIT-001` 和 `API-001`～`API-003` 均已完成。除上述唯一当前任务 `API-004` 外，不并行启动后续任务或新前端设计实现。API-004 必须避免单进程限流伪装为多实例保护，并且不提前实现 API-005 的预算熔断。前端整体重做继续冻结，只有在工程、API、数据、测试与 AI 可用性门禁全部满足后，才进入新前端专项讨论与建设。
