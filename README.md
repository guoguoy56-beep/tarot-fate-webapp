...
GitHub Actions 会在推送到 `main` 或向 `main` 发起 Pull Request 时执行质量门禁：固定 Node.js 24 与 npm `11.19.1`，运行干净的 `npm ci`，随后依次执行 high/critical 全量依赖审计、`npm test`、`npm run lint`、`npm run typecheck` 和 `npm run build`。npm 缓存只用于加速下载，不替代锁文件安装。

`npm test` 使用 Vitest 在 Node 环境运行 8 个文件、79 项领域单元测试，覆盖 78 张生产卡牌和真实 JPG、Fisher–Yates 与正逆位、问题字符/请求字节限制、请求可信重建和错误码、阅读响应/API 错误载荷，以及 localStorage 结构校验与 20 条上限。开发时可运行 `npm run test:watch`。可单独运行 `npm run audit:all` 检查全部依赖、`npm run audit:prod` 检查生产依赖；`npm run audit:signatures` 用于人工核验注册表签名和来源证明，不作为每次提交的硬门禁。项目通过 `.npmrc` 和 `package.json#allowScripts` 管理安装脚本审批。DeepSeek Mock、API 集成和浏览器 E2E 将继续在 TEST-003～006 按顺序接入。
...
截至 2026-09-22，阶段 1 与当前本地范围内的阶段 2 已完成，阶段 3 正在推进。`BASE-001～BASE-005`、`KIT-001`、`API-001～API-003`、`API-005`、`DATA-001～DATA-002`、`TEST-001～TEST-002` 已完成；`API-004` 的公网部署与匿名限流工作已移至未来发布阶段，只有用户明确决定公开部署后才重新评估。当前下一项是 `TEST-003` DeepSeek 适配器 Mock 测试。旧前端仍处于维护冻结期，整体前端重做保留在 `FE-GATE` 决策门之后。
